...
sealed interface AdvertisingInfoState {
    ...
    data class Initialized(val advertisingProfile: AdvertisingInfo.AdvertisingProfile) : AdvertisingInfoState
}
object AdvertisingInfo {
    ...
    suspend fun getAdvertisingProfile(context: Context): AdvertisingProfile =
        withContext(Dispatchers.IO) {
            if (state.compareAndSet(expect = AdvertisingInfoState.NotInitialized, update = AdvertisingInfoState.Initializing)) {
                fetchAdvertisingProfile(context)
            }
            val profile = state.first { it is AdvertisingInfoState.Initialized } as? AdvertisingInfoState.Initialized
    ...
    private fun getDefaultProfile(context: Context) = DefaultAdvertisingProfile.apply { extractParams(context) }
